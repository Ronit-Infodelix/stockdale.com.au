  ...
  return (
    <section className="relative w-full py-12 md:py-16 lg:py-20 bg-[#f5f5f5] overflow-hidden">
      ...
      <Container className="z-100 relative">
        {/* Heading + description */}
        <div className="flex flex-col lg:flex-row items-start justify-between gap-8 lg:gap-16 mb-10 lg:mb-12">
          {/* Left — eyebrow + heading + CTA */}
          <div className="flex flex-col gap-5 shrink-0 max-w-full lg:max-w-[420px]">
            <motion.h2
              className="font-agatho font-bold text-[36px] sm:text-[44px] lg:text-[48px] leading-[1.1] text-black"
          ...
          <motion.div
            className="flex flex-col gap-4 max-w-full lg:max-w-[500px] pt-1"
            initial={{ opacity: 0, x: 40 }}
            whileInView={{ opacity: 1, x: 0 }}
            viewport={VIEWPORT}
        ...
        >
          {stats.map(({ icon: Icon, value, suffix, label }) => (
            <motion.div
              key={label}
              variants={cardItem}
              className="bg-white rounded-[20px] border border-[#e5bd1b]/50 shadow-[0px_8px_10px_0px_rgba(0,0,0,0.04)] p-5 sm:p-6 flex flex-col justify-between h-[160px] sm:h-[190px]"
            >
              {/* Icon + number */}
              <div className="flex items-center gap-3">
                ...
                <CountAnimation
                  number={value}
                  suffix={suffix ?? ""}
                  className="font-agatho font-bold text-[40px] sm:text-[52px] leading-none text-brand-green-darkest"
                />{" "}
              </div>
